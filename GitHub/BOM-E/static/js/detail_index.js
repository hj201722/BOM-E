document.addEventListener("DOMContentLoaded", function () {
  ...
  db.ref("products_digital/" + productName)
    ...
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        // 가까운 위치에 있는 상품 출력
        fetchNearbyProducts(productName);
      } else {
  ...
  function fetchNearbyProducts(productName) {
    ...
    db.ref("products_digital")
      ...
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          const currentIndex = productKeys.indexOf(productName);

          if (currentIndex !== -1) {
            const start = Math.max(currentIndex - 5, 0);
            const end = Math.min(currentIndex + 5, productKeys.length - 1);
            ...
            console.log("Current product not found in the database.");
          }
        } else {
  ...
  function createProductCard(productName, productData) {
    ...
    const imgUrl = productData.img_url
      ? productData.img_url
      : "{% static 'img/shop/default.jpg' %}";
    const productDetailsUrl = `/product/${encodeURIComponent(productName)}/`;
